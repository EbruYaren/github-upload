...
class DerincurrencyKafkaResourceIT {
    ...
    void setup() {
        ...
        DerincurrencyKafkaResource kafkaResource = new DerincurrencyKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/derincurrency-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/derincurrency-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
